...
class Qwen2SdpaAttention(Qwen2Attention):
    ...
    def forward(
        ...
        past_key_value: Optional[Cache] = None,
        output_attentions: bool = False,
        tmp_score: Optional[torch.Tensor] = None,  # 新增参数
        use_cache: bool = False,
        cache_position: Optional[torch.LongTensor] = None,
        position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,  # will become mandatory in v4.46
        ...
        if tmp_score is not None:
            # Validate tmp_score shape
            assert tmp_score.shape == (bsz, self.num_heads, q_len, q_len), \
                f"tmp_score must have shape (bsz, num_heads, q_len, q_len), but got {tmp_score.shape}"
            # Directly use tmp_score as attention probabilities
            attention_probs = tmp_score
            attn_output = torch.matmul(attention_probs, value_states.transpose(1, 2))
            attn_output = attn_output.transpose(1, 2).contiguous()
        else:
